...
def get_angle(pos1, pos2):
    ...
    """
    angle = math.atan(- (pos2[1] - pos1[1]) / (pos2[0] - pos1[0])) if pos2[0] != pos1[0] else math.pi / 2
    return angle if pos2[0] > pos1[0] else math.pi + angle
...
def game():
    ...
    while not game_over:
        ...
        if time.time() - last_attack > 1.5 + random.random():
        ...
        for pixel in pixels:
            ...
            for shot in shots:
                if topx <= 300 + shot[1] * math.cos(shot[0]) <= bottomx \
                        and topy <= 300 - shot[1] * math.sin(shot[0]) <= bottomy:
                    pixels.remove(pixel)
                    shots.remove(shot)
                    score += 1
